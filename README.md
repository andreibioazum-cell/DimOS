...
- `DEL ИМЯ` — безопасно скрыть пользовательский файл в текущем сеансе. `KERNEL.BIN` защищён и удалить его нельзя.
...
В образе есть `KERNEL.BIN` и обычный пользовательский `README.TXT`. Файловый менеджер читает FAT12-каталог и данные с диска; системный файл защищён, поэтому удаление файлов не ломает загрузку. `DEL` намеренно действует только до перезагрузки: запись секторов пока не выполняется.
